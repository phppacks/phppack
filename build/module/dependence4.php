 ...
 * @Last Modified time: 2018-11-19 11:49:38
 */
...
TPL;
    ...
    public function getTraceFile($fileName = '', $fileAlias = '', $parentPath = "") {
        ...
        if ($extension) {
            ...
            if (in_array($fileName, $dependencies)) {
                $fileName = "./plugin/" . $fileName . ".js";
            }
            #判断处理非插件依赖关系
            else {
                #保存原名
     ...
     * @return [type]        [description]
     */
    private function getCompontentInfo($oneself = '', $parent = '') {
        #读取扩展名
        ...
        // }
        // if (stripos($file, "../") == 0) {
        //     $file = dirname($path) . DS . trim($file, "../");
        // }
        P($alias, $file);
        #判断是否是../目录
        if (stripos($alias, "../") === 0) {
            $alias = substr($file, 3);
            $file  = dirname($path) . DS . $alias;
        } else {
            if ($path !== '.' && stripos($file, "/") !== 0) {
                $file = $path . trim($file, '.');
            }
        }

        #判断是否为路由
        if ($alias == $file && $file == "phpRouter" && $this->is_router) {
            $file = $this->phpRouter;
        }
        if ($alias == $file && $file == "sonRouter" && $this->is_router) {
            $file = $this->sonRouter;
        }
        #如果文件有扩展名
        if ($extension) {
     ...
     * @return [type]            [description]
     */
    private function getStyleModules($value = '') {
        #获取样式信息
     ...
     * @return [type]            [description]
     */
    private function getScriptModules($value = '') {
        #获取唯一码
        $code = md5_file($value);
        #存储样式模块
        $this->styleModules[$code] = array(
